...
@Section('content')
            ...
            <table class="min-w-full border-collapse bg-white">
                <thead>
                    <tr class="bg-gradient-to-r h-12 from-sky-600 to-sky-500">
                        ...
                        <th class="px-6 py-3 text-nowrap  text-center text-md font-semibold text-white">ราคา</th>
                            ...
                            <td class="px-6 py-4 text-nowrap text-center text-xl text-gray-700">
                                <div class="flex justify-center gap-2">
                                    <a class="px-4 py-2 bg-yellow-300 text-black rounded hover:bg-yellow-600"
                                        href="{{ route('manager.products.store', $product->id) }}">Edit</a>
                                    <form action="{{ route('manager.products.destroy', $product->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-900"
    ...
    <div id="modal" class="fixed inset-0 p-2 bg-gray-900 bg-opacity-50 flex items-center justify-center hidden z-50">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden">
            ...
            <!-- Modal Body -->
            <div class="px-6 py-4">
                <form action="{{ route('manager.products.store') }}" id="productForm" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label for="price" class="block text-sm font-medium text-gray-700 mb-1">ราคาขาย</label>
                        ...
                        <div>
                            <label for="restock_level" class="block text-sm font-medium text-gray-700 mb-1">Restock
                                Level</label>
                            <input type="number" name="restock_level" id="restock_level"
                    ...
                    <div class="mt-4">
                        <label for="category_id" class="block text-sm font-medium text-gray-700 mb-1">ประเภทสินค้า</label>
    ...
    <div id="editModal" class="fixed inset-0 p-2 bg-gray-900 bg-opacity-50 flex items-center justify-center hidden z-50">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden">
            ...
            <!-- Modal Body -->
            <div class="px-6 py-4">
                <form action="{{ route('manager.products.store') }}" id="productForm2" method="POST"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label for="restock_level" class="block text-sm font-medium text-gray-700 mb-1">restock_level</label>
    ...
    <script>
        function openEditModal(id, name, price, costPrice, stockQuantity, restock_level, description, categoryId) {
            // เปิด Modal
            ...
            form.action = `/manager/products/update/${id}`; // เปลี่ยน action ของฟอร์ม

            document.getElementById('name2').value = name;
            document.getElementById('price2').value = price;
            document.getElementById('cost_price2').value = costPrice;
            document.getElementById('stock_quantity2').value = stockQuantity;
            document.getElementById('restock_level2').value = restock_level;
            document.getElementById('description2').value = description;
            document.getElementById('category_id2').value = categoryId;
            ...
            currentData.forEach((product, index) => {
                const row = `
                       <tr>
                           ...
                           <td class="px-6 py-2 text-nowrap  text-center text-md text-gray-700">${product.price}</td>
                           <td class="px-6 py-2 text-nowrap  text-center text-md text-gray-700">${product.stock_quantity}</td>
                           <td class="px-6 py-2 text-nowrap  text-center text-md text-gray-700">${product.category?.name || 'N/A'}</td>
                           <td class="px-6 py-2 text-nowrap  text-center text-md text-gray-700">
                               <div class="flex justify-center gap-2">
                                   <button 
                                       ...
                                       onclick="openEditModal(${product.id}, '${product.name}', ${product.price}, ${product.cost_price}, ${product.stock_quantity}, ${product.restock_level}, '${product.description}', '${product.category_id}')">
                                       Edit
                                   </button>
                                   <form id="deleteForm-${product.id}" action="/manager/products/destroy/${product.id}" method="POST">
