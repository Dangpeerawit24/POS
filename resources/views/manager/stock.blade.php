            ...
            <tbody>
                @foreach($products as $product)
                    <tr>
                        ...
                        <td class="border text-nowrap border-gray-300 px-4 py-2 text-right">{{ $product->stock_quantity }}</td>
                        ...
                        <td class="border text-nowrap border-gray-300 px-4 py-2 text-center">
                            <form action="{{ route('stock.add', $product->id) }}" method="POST" class="inline-block">
                                @csrf
                                <input type="number" name="quantity" class="w-16 sm:w-24 px-2 py-1 border rounded-lg" placeholder="จำนวน">
                                <button type="submit" class="sm:w-10 md:w-16 px-2 py-1 bg-green-500 text-white rounded-lg text-sm sm:text-base">เพิ่ม</button>
                            </form>
                            <form action="{{ route('stock.reduce', $product->id) }}" method="POST" class="inline-block ml-2">
                                @csrf
                                <input type="number" name="quantity" class="w-16 sm:w-24 px-2 py-1 border rounded-lg" placeholder="จำนวน">
                                <button type="submit" class="sm:w-10 md:w-16 px-2 py-1 bg-red-500 text-white rounded-lg text-sm sm:text-base">ลด</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
